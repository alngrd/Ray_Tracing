...
public class Scene {
	...
	private Vec calcColor(Ray ray, int recursionLevel) {
		...
		Hit bestIntersection = closestSurface.intersect(ray);
		Point intersectionPoint = ray.getHittingPoint(bestIntersection);
		Vec ambientComp = (closestSurface.Ka().mult(ambient));
		Vec reflectionRecurse;
		Vec refractionRecurse;
		...
		for(Light lightSource : lightSources){
			...
			if(rayReachesLight){
				...
				Vec rayToLightSourceNormal = bestIntersection.getNormalToSurface();

				lightComp.add(calcDiffuseColor(closestSurface, rayToLightSourceDirection, rayToLightSourceNormal)).mult(lightIntensity);
				lightComp.add(calcSpecularColor(closestSurface, rayToLightSourceDirection, rayToLightSourceNormal, ray.direction())).mult(lightIntensity);
			}
		}

		reflectionRecurse = (renderReflections && closestSurface.isReflecting()) ?
				calcColor(getReflectingRay(intersectionPoint, ray.direction(), bestIntersection.getNormalToSurface()), recursionLevel + 1) : new Vec(0);
		refractionRecurse = (renderRefractions && closestSurface.isTransparent()) ?
				calcColor(getRefractingRay(intersectionPoint, ray.direction(), bestIntersection.getNormalToSurface(),
						closestSurface.n1(bestIntersection), closestSurface.n2(bestIntersection)), recursionLevel + 1) : new Vec(0);

		return lightComp.add(ambientComp).add(closestSurface.Kr().mult(reflectionRecurse)).add(closestSurface.Kt().mult(refractionRecurse));
	}
